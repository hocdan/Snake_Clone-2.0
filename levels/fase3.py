'''
    ...
    LEVEL 3 - arquivo que carrega a primeira fase do jogo 
...
class Fase3(State):
    ...
    def onEnter(self, game):
        ...
        self.player = Snake(posX=8, posY=56, dir='d', Dx=8, vidas=self.dados[0], pontos=self.dados[1], tipo="planta")
        self.world = World_SNAKE(pyxel.tilemaps[0], largura=16, altura=16, baseX=32, baseY=0, tamanho=8, SNAKE=self.player)
        self.INPUT = 'd' #flag de controle para a direcao passiva da cobra (movimento inicial)
        ...
        if (not self.GAME_OVER):
            ...
            if (pyxel.frame_count%20 == 0):
                if ( self.world.checkCollision(self.INPUT) == "comida"):
                    ...
                    pyxel.play(0, 14) #reproduzindo sound[1] no channel[0] (efeito PONTOS)
                    ...
                    #checando caso especial: jogador com orb de fogo no gelo
                    if (self.world.checkCollision(self.INPUT) == "cubo" and self.player.tipo == "fogo"):
                        self.player.vidas -= 1 #dano dobrado por fraqueza
                    #checando caso especial: jogador com orb de areia colidindo com estruturas
