'''
    ...
    LEVEL 5 (nivel final) - arquivo que carrega a ultima fase do jogo 
...
class Fase5(State):
    ...
    def onEnter(self, game):
        ...
        self.player = Snake(posX=440, posY=8, dir='s', Dx=8, vidas=self.dados[0], pontos=self.dados[1], tipo="planta")
        self.world = World_SNAKE(pyxel.tilemaps[0], largura=64, altura=16, baseX=0, baseY=16, tamanho=8, SNAKE=self.player)
        self.world.printWorld() #TESTEEEEEEE!!!!
        ...
        #declarando variaveis de controle de renderizacao movel do mapa
        self.larguraAtual = (48, 64)
        self.posXAtual = 456
        ...
        if (not self.GAME_OVER):
            ...
            if (pyxel.frame_count%self.time == 0):
                if ( self.world.checkCollision(self.INPUT) == "comida"):
                    ...
                    #encerrando musica da fase4 e iniciando musica da fase1
                    ...
                    if (self.time <= 4):
                        ...
                        self.time -= 4 #acelerando ritmo do jogo
                    self.player.move(self.INPUT, LARGURA_JANELA, ALTURA_JANELA)
                elif ( self.world.checkCollision(self.INPUT) == "relogioDOWN"):
                    pyxel.play(0, 1) #reproduzindo sound[1] no channel[0] (efeito PONTOS)
                    self.time += 4 #reduzindo ritmo do jogo
                    self.player.move(self.INPUT, LARGURA_JANELA, ALTURA_JANELA)
    ...
    def draw(self, game):
        ...
        if (not self.GAME_OVER):
            ...
            #determinando em qual pedaco do mapa o jogador esta para entao desenhar essa porcao...
            if (self.player.corpo[0].posX>=0 and self.player.corpo[0].posX <= 127):
                self.larguraAtual = (0, 16)
            elif (self.player.corpo[0].posX>=128 and self.player.corpo[0].posX <= 255):
                self.larguraAtual = (16, 32)
            elif (self.player.corpo[0].posX>=256 and self.player.corpo[0].posX <= 383):
                self.larguraAtual = (32, 48)
            elif (self.player.corpo[0].posX>=384 and self.player.corpo[0].posX <= 511):
                self.larguraAtual = (48, 64)
            #desenhando mapa (na range de: 128 pixels/16 tiles da localizacao atual do jogador)
            for y in range(self.world.ALTURA):
                for x in range(self.larguraAtual[0], self.larguraAtual[1]):
                    worldItem = self.world.world_map[y][x]
                    self.world.draw_worldItens(pyxel, x-self.larguraAtual[0], y, 0, worldItem)
